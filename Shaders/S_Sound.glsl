...
float getfrequency(float x) {
    float texCoordX = (floor(x * FREQ_RANGE + 1.0) / FREQ_RANGE);
    return texture(audioTexture, vec2(texCoordX, 0.25)).r + 0.06;
}
...
void main() {
    ...
    vec2 fragPos = fragCoord / screen;
    fragPos = (fragPos - 0.5) * 2.0;
    fragPos.x *= screen.x / screen.y;
