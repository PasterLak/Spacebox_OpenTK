...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{ vec2 x = TexCoords * tiling + offset;
    ...
    //combine results
    vec3 ambient  = light.ambient  * vec3(texture(material.diffuse, x));
    vec3 diffuse  = light.diffuse  * diff * vec3(texture(material.diffuse, x));
    vec3 specular = light.specular * spec * vec3(texture(material.specular, x));
