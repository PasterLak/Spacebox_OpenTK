...
public class CraftingGUI
{
    ...
    private static void OnClick(Blueprint blueprint)
    {
        ...
        {
            var item = bp.Product.Item;
            var total = bp.Product.Quantity;
            var isGear = item.Is<WeaponItem>() || item.Is<DrillItem>();

            var (primary, secondary) = isGear ? (Panel, Inventory) : (Inventory, Panel);

            primary.TryAddItem(item, total, out var rest);

            if (rest > 0)
            {
                secondary.TryAddItem(item, rest, out rest);
            }

            currentPlayer.PlayerStatistics.ItemsCrafted += (total - rest);
        }
        else
        {
    ...
    public static void SlotWithItem(string label, Vector2 pos, float width, float height, Action<Blueprint> onClick, int slotId)
    {
        ...
        if (itemData.item.IconTextureId != IntPtr.Zero)
        {
            if (canCraft)
            {
                var count = CraftingLogic.CalculatePossibleItemCraftCount(Inventory, Panel, itemData.blueprint, currentPlayer);
