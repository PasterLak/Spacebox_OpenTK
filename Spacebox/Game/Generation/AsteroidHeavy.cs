...
namespace Spacebox.Game.Generation
{
    public class AsteroidHeavy : Asteroid
    {
        ...
        public AsteroidHeavy(ulong id, Vector3 positionWorld, Sector sector)
        ...
        private void GenerateVoxelData()
        {
            ...
            var oreParams = new AsteroidOreGeneratorParameters(
                outerOreVeinChance: 0.02f, outerOreMaxVeinSize: 12, outerOreIds: new[] { 4, 5, 6, 7 },
                middleOreVeinChance: 0.015f, middleOreMaxVeinSize: 10, middleOreIds: new[] { 8, 9, 11 },
                deepOreVeinChance: 0.01f, deepOreMaxVeinSize: 1, deepOreIds: new[] { 10 },
                oreSeed: (int)Seed
            );
            new AsteroidOreGenerator(oreParams).ApplyOres(ref _voxelData, Spacebox.Generation.AsteroidType.Heavy);
        }
            ...
            for (int x = 0; x < Chunk.Size; x++)
                for (int y = 0; y < Chunk.Size; y++)
                    for (int z = 0; z < Chunk.Size; z++)
                    {
                        ...
                        if (gx < 0 || gy < 0 || gz < 0 ||
                            gx >= _gridSize || gy >= _gridSize || gz >= _gridSize)
