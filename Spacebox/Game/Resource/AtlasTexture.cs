...
public partial class AtlasTexture : IDisposable
{
    public int BlockSizePixels { get; private set; }
    public int SizeBlocks { get; private set; }

    private Dictionary<Texture2D, AtlasTextureData> Textures = new();
    private Dictionary<string, AtlasTextureData> ReadyTextures = new();
    ...
    public Texture2D CreateTexture(string path, TextureData[]? additional, int blockSizePixels, bool populateFromTopToBottom)
    {
        ...
        foreach (var tex in textures)
        {
            ...
            {
                AtlasTextureData info = new AtlasTextureData();
                info.Name = tex.Name;
        ...
        for (int y = 0; y < sideBlocks; y++)
        {
            for (int x = 0; x < sideBlocks; x++)
            {
                ...
                if (textureIndex < textures.Count)
                {
                    ...
                    AtlasTextureData atlasTextureInfo = Textures[textures[textureIndex]];
